...
const LogIn = () => {
    ...
    const handleLogin = async (event) => {
        ...
        if (phoneNumber === "admin" && password === "admin123") {
        ...
        } else if (phoneNumber === "9747712991" && password === "sabysaji") {
            const staffId = "6644da30702cb4bb0812be94"
            navigate("/reqstaff", { state: { staffId } });
        } else {
            try {
            ...
            try {
                ...
                const response = await axios.post(
                    `http://localhost:3500/staff/login-staff`,
                    requestData,
                    {
                        headers: {
                            "Content-Type": "application/json",
                ...
                if (response.status === 200) {
                    const responseData = response.data;
                    const staffId = responseData.loggedUserId;
...
//         const requestData = JSON.stringify({ phoneNumber, password });

//         try {
//             // Attempt login for general users
...
//             }
//         } catch (error) {
//             console.log("User login failed, trying staff login", error);
//             try {
...
//                     const responseData = staffResponse.data;
//                     const staffId = responseData.loggedUserId;
//                     navigate("/reqstaff", { state: { staffId } });
//                 } else {
...
//                 setError("Failed to login as user or staff");
//             }
//         }
//     };
